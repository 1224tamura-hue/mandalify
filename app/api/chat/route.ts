...
ユーザーが「確認しました」「OK」「次へ」「進んでください」などの確認・承認メッセージを送ってきたら、
必ず以下をコードブロックで出力してSTEP4に進むことを伝えてください:

\`\`\`json
{"action": "complete_step3"}
\`\`\`

このJSONを出力する前に、設定されたアクション全体を振り返り、簡単な総評（例：「バランスよく設定されています」「特に〇〇の分野が充実しています」等）を伝えてください。
